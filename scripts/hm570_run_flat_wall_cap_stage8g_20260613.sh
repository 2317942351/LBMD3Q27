...
# Run Stage8g flat-wall low-angle shadow diagnostics.
# Status: runtime_sanity / exploratory_not_validation.

source "/media/yuan/新加卷1/RUNS/scripts/tclb_runs_env.sh"
RUN_ROOT="${1:-$TCLB_RUNS_ROOT/stage8/stage8g_cap_contract_revision_diag_20260613/flat}"
LOCAL_CASE_STAGING="${LOCAL_CASE_STAGING:-/tmp/flat_wall_cap_stage8g_low_angle_cases}"
...
mkdir -p "$RUN_ROOT"
batch_log="$RUN_ROOT/batch_flat_wall_cap_stage8g.log"
...

{
  ...
  echo "purpose=flat_wall_spherical_cap_stage8g_shadow"
...
for helper in "$FINITENESS" "$CAP_POST" "$ATTRIBUTION"; do
