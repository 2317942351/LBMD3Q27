#!/usr/bin/env python3
"""Digest Stage14-B23/B24/B27 probe roots into compact comparison tables.

The input root should contain one subdirectory per probe. Each probe directory
is expected to contain b23_key_summary.json, b24_key_summary.json, or
b27_key_summary.json generated by stage14_b17_onset_mask_argmax.py. This script
only summarizes diagnostic evidence; it does not validate contact angle or
promote a physics mode.
"""
...
def summary_paths(root: Path) -> list[Path]:
    ...
    paths.extend(sorted(root.glob("*/b27_key_summary.json")))
    return paths
