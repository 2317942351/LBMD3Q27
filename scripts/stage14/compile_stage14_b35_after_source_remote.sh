...
LOG="${LOG:-/home/yuan/lbm2026_stage14_B35_compile_20260627.log}"
...

{
  ...
  grep -q "CudaConstantMemory real_t Stage14B35CoupledNumeratorDiagnosticsMode" "CLB/${TARGET}/cuda.cu"
  grep -q "#define B35FtotalLegacyX" "CLB/${TARGET}/cuda.cu"
  grep -q "#define B35ForceRhoRaw" "CLB/${TARGET}/cuda.cu"
  grep -q "#define B35ForceRhoDensityFloor" "CLB/${TARGET}/cuda.cu"
  grep -q "#define B35ForceRhoPhaseMixture" "CLB/${TARGET}/cuda.cu"
  grep -q "stage14_b35_coupled_numerator_diagnostics_active" "CLB/${TARGET}/Dynamics.c"
  grep -q "B35FtotalLegacyX = b35_total_legacy\\[0\\]" "CLB/${TARGET}/Dynamics.c"
  grep -q "CudaDeviceFunction vector_t getB35FtotalLegacy" "CLB/${TARGET}/Dynamics.c"
  grep -q "CudaDeviceFunction real_t getB35ForceRhoRaw" "CLB/${TARGET}/Dynamics.c"
  grep -q "real_t B35FtotalLegacyX" "CLB/${TARGET}/Dynamics.h"
  grep -q "real_t B35ForceRhoRaw" "CLB/${TARGET}/Dynamics.h"
  grep -q "SETTINGS_Stage14B35CoupledNumeratorDiagnosticsMode" "CLB/${TARGET}/Consts.h"
  grep -q '"B35FtotalLegacy"' "CLB/${TARGET}/Lists.cpp"
  grep -q '"B35ForceRhoRaw"' "CLB/${TARGET}/Lists.cpp"
  grep -q "GetB35FtotalLegacy" "CLB/${TARGET}/Lattice.cu"
  grep -q "GetB35FtotalLegacy" "CLB/${TARGET}/Solver.cpp"
  grep -q "load_B35FtotalLegacyX" "CLB/${TARGET}/LatticeAccess.inc.cpp"
  grep -q "load_B35ForceRhoRaw" "CLB/${TARGET}/LatticeAccess.inc.cpp"
  echo "CUDA_B35_SLIM_ACCESSORS_OK=1"
