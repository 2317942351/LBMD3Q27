...
ROOT="${ROOT:-/mnt/usb1t/RUNS/runs/stage14_B35_coupled_numerator_split_20260627}"
...
{
  echo "stage=stage14_B35_coupled_numerator_split"
  echo "root=${ROOT}"
  echo "gpu=${GPU}"
  echo "binary=${BIN}"
  echo "iterations=${ITERATIONS}"
  echo "vtk_period=${VTK_PERIOD}"
  echo "vtk_field_set=b35split"
  echo "density_h=1.0"
  ...
  echo "mu_cap=${MU_CAP}"
  echo "grad_cap=${GRAD_CAP}"
  echo "force_over_rho_denominator_sweeps=offline_from_B35Ftotal_and_B35ForceRho_fields"
  echo "claim_limit=shadow_only_coupled_numerator_split_not_solver_fix"
...
run_probe() {
  ...
  echo "RUN_B35 tag=${tag} MomentumForceMode=${momentum_force_mode}"
  set +e
  "${PY}" "${SCRIPT}" \
    --root "${probe_root}" \
    --binary "${BIN}" \
    ...
    --b35-coupled-numerator-diagnostics-mode 1 \
    --b35-mu-abs-cap "${MU_CAP}" \
    --b35-grad-phi-cap "${GRAD_CAP}" \
  ...
  "${PY}" "${ANALYZER}" "${probe_root}" --out-dir "${probe_root}" --prefix b35 \
...
set +e
"${PY}" "${DIGEST}" "${ROOT}" \
  --csv "${ROOT}/b35_matrix_digest.csv" \
  --json "${ROOT}/b35_matrix_digest.json" \
  > "${ROOT}/b35_matrix_digest_stdout.json" \
  2> "${ROOT}/b35_matrix_digest_stderr.log"
digest_rc=$?
set -e
...
echo "OVERALL_RC=${overall_rc}" | tee "${ROOT}/stage14_B35.status"
exit "${overall_rc}"
