...
ROOT="${ROOT:-/mnt/usb1t/RUNS/runs/stage14_B31_force_density_candidate_20260627}"
...
FORCE_DENSITY_MODE="${FORCE_DENSITY_MODE:-2}"
...
{
  echo "stage=stage14_B31_force_density_candidate"
  echo "root=${ROOT}"
  echo "gpu=${GPU}"
  echo "binary=${BIN}"
  echo "fmu_stress_closure_mode=${FMU_STRESS_MODE}"
  echo "phase_advection_velocity_mode=${PHASE_ADV_MODE}"
  echo "force_density_closure_mode=${FORCE_DENSITY_MODE}"
  echo "density_h=1.0"
  echo "density_l=0.005"
  echo "claim_limit=force_density_candidate_only_not_contact_angle_validation"
...
run_probe() {
  ...
  echo "RUN_B31 tag=${tag} iterations=${iterations} ForceDensityClosureMode=${FORCE_DENSITY_MODE} FmuStressClosureMode=${FMU_STRESS_MODE} PhaseAdvectionVelocityMode=${PHASE_ADV_MODE}"
  set +e
  "${PY}" "${SCRIPT}" \
    --root "${probe_root}" \
    --binary "${BIN}" \
  ...
  "${PY}" "${ANALYZER}" "${probe_root}" --out-dir "${probe_root}" --prefix b31 \
...
overall_rc=0
run_probe B31_020 20 1 || overall_rc=3
if [[ "${overall_rc}" -eq 0 ]]; then
  run_probe B31_100 100 5 || overall_rc=3
fi

set +e
"${PY}" "${DIGEST}" "${ROOT}" \
  --csv "${ROOT}/b31_matrix_digest.csv" \
  --json "${ROOT}/b31_matrix_digest.json" \
  > "${ROOT}/b31_matrix_digest_stdout.json" \
  2> "${ROOT}/b31_matrix_digest_stderr.log"
digest_rc=$?
set -e
...
echo "OVERALL_RC=${overall_rc}" | tee "${ROOT}/stage14_B31.status"
exit "${overall_rc}"
