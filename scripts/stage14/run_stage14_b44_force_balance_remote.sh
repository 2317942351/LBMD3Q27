...
ROOT="${ROOT:-/mnt/usb1t/RUNS/runs/stage14_B44_force_balance_20260628}"
PY="${PY:-/usr/bin/python3}"
SCRIPT="${SCRIPT:-/home/yuan/stage14_s2_replay_smoke.py}"
RANKER="${RANKER:-/home/yuan/stage14_b44_force_balance_rank.py}"
...
{
  echo "stage=stage14_B44_force_balance"
  echo "root=${ROOT}"
  echo "gpu=${GPU}"
  echo "binary=${BIN}"
  ...
  echo "claim_limit=force_balance_postprocess_not_solver_repair"
...
"${PY}" "${SCRIPT}" \
  ...
  --force \
  --run \
  > "${ROOT}/b44_driver_stdout.json" \
  2> "${ROOT}/b44_driver_stderr.log"
driver_rc=$?
echo "DRIVER_RC=${driver_rc}" > "${ROOT}/b44_driver.status"

"${PY}" "${RANKER}" "${ROOT}/B44_force_balance" \
  --out-dir "${ROOT}/B44_force_balance" \
  --prefix b44 \
  --focus-step "${FOCUS_STEP}" \
  > "${ROOT}/b44_ranker_stdout.json" \
  2> "${ROOT}/b44_ranker_stderr.log"
ranker_rc=$?
echo "RANKER_RC=${ranker_rc}" > "${ROOT}/b44_ranker.status"
set -e
...

{
  ...
  if [[ "${overall_rc}" -eq 0 ]]; then
    echo "VERDICT=b44_force_balance_complete"
  else
    echo "VERDICT=b44_force_balance_failed"
  fi
} | tee "${ROOT}/stage14_B44.status"
