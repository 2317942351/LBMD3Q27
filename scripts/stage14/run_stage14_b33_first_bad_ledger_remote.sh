...
ROOT_BASE="${ROOT_BASE:-/mnt/win_sda2/RUNS/runs}"
if [[ -d "${ROOT_BASE}" && -w "${ROOT_BASE}" ]]; then
  DEFAULT_ROOT="${ROOT_BASE}/stage14_B33_first_bad_ledger_20260627"
else
  DEFAULT_ROOT="/home/yuan/runs/stage14_B33_first_bad_ledger_20260627"
fi
...
{
  echo "stage=stage14_B33_first_bad_ledger"
  echo "root=${ROOT}"
  echo "gpu=${GPU}"
  echo "binary=${BIN}"
  ...
  echo "claim_limit=first_bad_cell_ledger_only_not_contact_angle_or_dynamic_validation"
} | tee "${ROOT}/run_config.txt"
sha256sum "${BIN}" | tee "${ROOT}/binary_sha256.txt"
nvidia-smi -L | tee "${ROOT}/nvidia_smi_L.txt"
df -h "${ROOT}" /mnt/usb1t /mnt/win_sda2 /home 2>&1 | tee "${ROOT}/df_before.txt"

run_probe() {
  local tag="$1"
  local momentum_force_mode="$2"
  local probe_root="${ROOT}/${tag}"
  echo "RUN_B33 tag=${tag} MomentumForceMode=${momentum_force_mode}"
  set +e
  "${PY}" "${SCRIPT}" \
    --root "${probe_root}" \
    --binary "${BIN}" \
  ...
  "${PY}" "${ANALYZER}" "${probe_root}" --out-dir "${probe_root}" --prefix b33 \
  ...
  if [[ "${KEEP_VTI}" != "1" ]]; then
    ...
    : > "${probe_root}_deleted_argmax_trace_manifest.txt"
  fi
  set -e
...
overall_rc=0
run_probe L0_full 0 || overall_rc=3
run_probe L1_noFmu 1 || overall_rc=3
run_probe L2_noSurf 3 || overall_rc=3
run_probe L3_noPressure 2 || overall_rc=3
run_probe L4_zeroForce 4 || overall_rc=3

set +e
"${PY}" "${DIGEST}" "${ROOT}" \
  --csv "${ROOT}/b33_matrix_digest.csv" \
  --json "${ROOT}/b33_matrix_digest.json" \
  > "${ROOT}/b33_matrix_digest_stdout.json" \
  2> "${ROOT}/b33_matrix_digest_stderr.log"
digest_rc=$?
set -e
...
df -h "${ROOT}" /mnt/usb1t /mnt/win_sda2 /home 2>&1 | tee "${ROOT}/df_after.txt"
echo "OVERALL_RC=${overall_rc}" | tee "${ROOT}/stage14_B33.status"
exit "${overall_rc}"
