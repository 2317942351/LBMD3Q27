...
ROOT="${ROOT:-/mnt/usb1t/RUNS/runs/stage14_B41_stress_source_audit_20260628}"
PY="${PY:-/usr/bin/python3}"
SCRIPT="${SCRIPT:-/home/yuan/stage14_s2_replay_smoke.py}"
ANALYZER="${ANALYZER:-/home/yuan/stage14_b17_onset_mask_argmax.py}"
RANKER="${RANKER:-/home/yuan/stage14_b41_stress_source_rank.py}"
...
{
  echo "stage=stage14_B41_stress_source_audit"
  echo "root=${ROOT}"
  echo "gpu=${GPU}"
  echo "binary=${BIN}"
  ...
  echo "instrumentation=B41 reuses B40 stress-source AddField diagnostics"
  echo "density_h=1.0"
  ...
  echo "claim_limit=stress_source_ranking_not_solver_repair"
...
set +e
"${PY}" "${SCRIPT}" \
  --root "${ROOT}/B41_stress_source_audit" \
  --binary "${BIN}" \
  --gpu "${GPU}" \
  ...
  --force \
  --run \
  > "${ROOT}/b41_driver_stdout.json" \
  2> "${ROOT}/b41_driver_stderr.log"
driver_rc=$?
echo "DRIVER_RC=${driver_rc}" > "${ROOT}/b41_driver.status"

"${PY}" "${ANALYZER}" "${ROOT}/B41_stress_source_audit" --out-dir "${ROOT}/B41_stress_source_audit" --prefix b41 \
  > "${ROOT}/b41_analyzer_stdout.json" \
  2> "${ROOT}/b41_analyzer_stderr.log"
analyzer_rc=$?
echo "ANALYZER_RC=${analyzer_rc}" > "${ROOT}/b41_analyzer.status"

"${PY}" "${RANKER}" "${ROOT}/B41_stress_source_audit/b41_mask_stats.csv" \
  --out-dir "${ROOT}/B41_stress_source_audit" \
  --prefix b41 \
  > "${ROOT}/b41_ranker_stdout.json" \
  2> "${ROOT}/b41_ranker_stderr.log"
ranker_rc=$?
echo "RANKER_RC=${ranker_rc}" > "${ROOT}/b41_ranker.status"
set -e
...

{
  ...
  if [[ "${overall_rc}" -eq 0 ]]; then
    echo "VERDICT=b41_stress_source_audit_complete"
  else
    echo "VERDICT=b41_stress_source_audit_failed"
  fi
} | tee "${ROOT}/stage14_B41.status"
