...
ROOT="${ROOT:-/mnt/usb1t/RUNS/runs/stage14_B29_short_stability_20260627}"
...
{
  echo "stage=stage14_B29_short_stability"
  echo "root=${ROOT}"
  echo "gpu=${GPU}"
  echo "binary=${BIN}"
  ...
  echo "claim_limit=short_stability_gate_only_not_contact_angle_validation"
...
run_probe() {
  ...
  echo "RUN_B29 tag=${tag} iterations=${iterations} FmuStressClosureMode=${FMU_STRESS_MODE}"
  set +e
  "${PY}" "${SCRIPT}" \
    --root "${probe_root}" \
    --binary "${BIN}" \
  ...
  "${PY}" "${ANALYZER}" "${probe_root}" --out-dir "${probe_root}" --prefix b29 \
...
overall_rc=0
run_probe B29_020 20 1 || overall_rc=3
if [[ "${overall_rc}" -eq 0 ]]; then
  run_probe B29_100 100 5 || overall_rc=3
fi

set +e
"${PY}" "${DIGEST}" "${ROOT}" \
  --csv "${ROOT}/b29_matrix_digest.csv" \
  --json "${ROOT}/b29_matrix_digest.json" \
  > "${ROOT}/b29_matrix_digest_stdout.json" \
  2> "${ROOT}/b29_matrix_digest_stderr.log"
digest_rc=$?
set -e
...
echo "OVERALL_RC=${overall_rc}" | tee "${ROOT}/stage14_B29.status"
exit "${overall_rc}"
