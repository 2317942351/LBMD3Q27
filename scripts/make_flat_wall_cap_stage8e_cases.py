#!/usr/bin/env python3
"""Generate Stage8e flat-wall low-angle cap diagnostic cases."""
...
CASE_DIR = Path("cases") / "diagnostics" / "flat_wall_cap_stage8e_low_angle_20260613"
REMOTE_ROOT = "/mnt/8A0E24070E23EAC1/runs/tclb_flat_wall_cap_stage8e_low_angle_20260613"
BINARY = (
    "/home/yuan/src/TCLB_stage8e_normal_residual_only_wetting_candidate_20260613/"
...
def cases_for_set(case_set: str) -> list[FlatCase]:
    ...
    if case_set == "write_low_angle":
        return [
            FlatCase(f"cap_theta030_wall{angle:03d}_mode2", 30.0, float(angle), 2.0)
            for angle in [5, 8, 11, 15, 20, 25, 30]
        ]
    if case_set == "wall011_modes":
        return [
            FlatCase("cap_theta030_wall011_mode1", 30.0, 11.0, 1.0),
            FlatCase("cap_theta030_wall011_mode2", 30.0, 11.0, 2.0),
        ]
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--case-set", choices=["shadow_low_angle", "write_low_angle", "wall011_modes"], default="shadow_low_angle")
...
<!--
  Stage8e flat-wall low-angle diagnostic.
  Status: {STATUS} / exploratory_not_validation.
  Stage8CandidateVersion=8.5; mode 1 shadow-only, mode 2 writes gradPhiVal.
...
def main() -> None:
    ...
    manifest = {
        ...
        "purpose": "Stage8e flat-wall low-angle diagnostic gate",
        "binary": args.binary,
