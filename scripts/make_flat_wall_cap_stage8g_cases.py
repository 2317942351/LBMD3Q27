#!/usr/bin/env python3
"""Generate Stage8g flat-wall low-angle shadow diagnostic cases."""
...
CASE_DIR = Path("cases") / "diagnostics" / "flat_wall_cap_stage8g_low_angle_20260613"
REMOTE_ROOT = "/media/yuan/新加卷1/RUNS/runs/stage8/stage8g_cap_contract_revision_diag_20260613/flat"
BINARY = (
    "/home/yuan/src/TCLB_stage8g_cap_contract_revision_diag_20260613/"
...
def cases_for_set() -> list[FlatCase]:
    return [
        FlatCase(f"cap_theta030_wall{angle:03d}_stage8g_mode{mode}", 30.0, float(angle), mode)
        for mode in [0, 1, 2, 3]
...
<!--
  Stage8g flat-wall low-angle shadow diagnostic.
...
<CLBConfig version="2.0" output="{remote_case}/output/" permissive="true">
  ...
  <Model>
    ...
    <Param name="Stage8gMode" value="{case.stage8g_mode}"/>
  ...
  <VTK what="{STAGE8G_VTK}"/>
  <Log Iterations="100"/>
  <Failcheck Iterations="100"/>
  <Solve Iterations="100">
    <VTK Iterations="100" what="{STAGE8G_VTK}"/>
    <Log Iterations="100"/>
    <Failcheck Iterations="100"/>
  </Solve>
  <Solve Iterations="900">
    <VTK Iterations="900" what="{STAGE8G_VTK}"/>
    <Log Iterations="900"/>
...
def main() -> None:
    ...
    manifest = {
        ...
        "purpose": "Stage8g flat-wall low-angle shadow diagnostic gate",
        "binary": args.binary,
