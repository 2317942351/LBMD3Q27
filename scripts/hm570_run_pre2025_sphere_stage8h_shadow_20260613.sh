...
# Launch Stage8h z48 sphere shadow diagnostics as two independent P100 queues.
...
RUN_ROOT="${1:-$TCLB_RUNS_ROOT/stage8/stage8h_contact_relation_and_profile_path_audit_20260613/sphere}"
CASE_STAGING="${CASE_STAGING:-/tmp/pre2025_sphere_stage8h_shadow_cases}"
...
QUEUE_NAME=sphere_a GPU_ID="$GPU_A" RUN_ROOT="$RUN_ROOT" CASE_STAGING="$CASE_STAGING" CASE_LIST="$queue_a" \
  CASE_KIND=sphere DELETE_RAW_AFTER_ANALYSIS="$DELETE_RAW_AFTER_ANALYSIS" DEFER_POSTPROCESS="$POSTPROCESS_POOL" "$QUEUE_SCRIPT" &
pid_a=$!
QUEUE_NAME=sphere_b GPU_ID="$GPU_B" RUN_ROOT="$RUN_ROOT" CASE_STAGING="$CASE_STAGING" CASE_LIST="$queue_b" \
  CASE_KIND=sphere DELETE_RAW_AFTER_ANALYSIS="$DELETE_RAW_AFTER_ANALYSIS" DEFER_POSTPROCESS="$POSTPROCESS_POOL" "$QUEUE_SCRIPT" &
pid_b=$!

if [[ "$POSTPROCESS_POOL" != "0" ]]; then
  python3 "$POSTPROCESS_SCRIPT" --run-root "$RUN_ROOT" --case-kind sphere --workers "$POSTPROCESS_WORKERS" \
