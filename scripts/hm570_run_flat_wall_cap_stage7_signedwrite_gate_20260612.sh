...
# Run Stage7 signed wall-ghost flat-wall gate cases.
# Status: runtime_sanity / exploratory_not_validation.
# Raw VTI/PVTI/PRI stay on HM570; no tar/archive is produced here.

RUN_ROOT="${1:-/mnt/8A0E24070E23EAC1/runs/tclb_flat_wall_cap_stage7_signedwrite_gate_20260612}"
LOCAL_CASE_STAGING="${LOCAL_CASE_STAGING:-/tmp/flat_wall_cap_stage7_signedwrite_gate_cases}"
BINARY="${BINARY:-/home/yuan/src/TCLB_clean_wall_signed_ghost_stage7diag_20260611/CLB/d3q27_pf_velocity_q27_geometric/main}"
...
FINITE_ARRAYS="PhaseField,U,P,Rho,WallPhasePred,WallPhaseProfilePred,WallPhaseSignedProfilePred,WallPhaseRawPred,WallPhaseSignedPred,WallContactResidual,WallSignedNormalGrad,WallTangentGradMag,WallSignedDeltaQ,WallSignedQClipped,BoundaryMask"
...
mkdir -p "$RUN_ROOT"
batch_log="$RUN_ROOT/batch_flat_wall_cap_stage7_signedwrite_gate.log"
current_case_dir=""
...

{
  ...
  echo "status=runtime_sanity"
  echo "purpose=flat_wall_spherical_cap_gate_stage7_signedwrite"
