#!/usr/bin/env python3
"""Postprocess Stage8g cap-contract / low-angle shadow diagnostic VTI frames."""
...
def decision(rows: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    mode = final.get("stage8g_mode", math.nan)
    ...
    blocked = (
        not np.isfinite(normal_limiter_fraction)
        or normal_limiter_fraction > 0.05
        or (np.isfinite(vector_limiter_fraction) and vector_limiter_fraction > 0.001)
        or (np.isfinite(outer90_normal_limiter) and outer90_normal_limiter > 0)
        or (np.isfinite(ratio_p50) and ratio_p50 < 0.9)
    )
    root_cause = "undetermined_shadow_only"
    if rows:
        sphere_hits = final.get("sphere11_normal_limiter_count", 0)
        total_hits = final.get("normal_limiter_count", 0)
        outer_hits = final.get("outer90_normal_limiter_count", 0)
        if total_hits and sphere_hits / total_hits > 0.9 and outer_hits == 0:
            root_cause = "sphere11_low_angle_candidate_dominated"
        if np.isfinite(normal_limiter_fraction) and normal_limiter_fraction > 0.05:
            root_cause += "_normal_limiter_dominated"
        if np.isfinite(ratio_cap_limiter_fraction) and ratio_cap_limiter_fraction > 0.5:
            root_cause += "_ratio_cap_contract_dominated"
        elif np.isfinite(abs_cap_limiter_fraction) and abs_cap_limiter_fraction > 0.5:
            root_cause += "_abs_cap_dominated"
    ...
    gate_pass = (
        rows
        and np.isfinite(normal_limiter_fraction)
        and normal_limiter_fraction <= 0.10
        and (not np.isfinite(vector_limiter_fraction) or vector_limiter_fraction <= 0.001)
        and (not np.isfinite(outer90_normal_limiter) or outer90_normal_limiter == 0)
        and (not np.isfinite(ratio_p50) or ratio_p50 >= 0.8)
        and (not np.isfinite(cap_demand_p50) or cap_demand_p50 < 1.2)
    )
    if gate_pass:
        if mode == 1:
            root_cause = "stage8g_mode1_cap_contract_revision_shadow_gate_passed"
        elif mode == 2:
            root_cause = "stage8g_mode2_low_angle_tan_regularization_shadow_gate_passed"
        elif mode == 3:
            root_cause = "stage8g_mode3_combined_cap_and_tan_shadow_gate_passed"
        else:
            root_cause = "stage8g_mode0_baseline_shadow_gate_passed"
    return {
        "status": STATUS,
        "claim_limit": "runtime_sanity / exploratory_not_validation only",
        "write_mode_allowed": False,
        "write_mode_blocked": True,
        "block_reason": "Stage8g is shadow-only; sphere Stage8OperatorMode=2 remains forbidden",
        "shadow_gate_passed_for_planning": bool(gate_pass),
        "root_cause_classification": root_cause,
        "final_stage8g_mode": mode,
...
def main() -> None:
    ...
    (args.out_dir / "stage8g_shadow_attribution_summary.json").write_text(
        json.dumps(payload, indent=2), encoding="utf-8"
    )
    write_csv(args.out_dir / "stage8g_shadow_attribution_summary.csv", [flatten_summary(r) for r in summaries])
    write_csv(args.out_dir / "stage8g_shadow_attribution_bins.csv", bin_rows_all)
