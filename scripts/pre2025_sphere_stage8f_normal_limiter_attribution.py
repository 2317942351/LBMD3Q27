#!/usr/bin/env python3
"""Postprocess Stage8f normal-limiter root-cause diagnostic VTI frames."""
...
def summarize_frame(path: Path) -> tuple[dict[str, Any], list[dict[str, Any]]]:
    ...
    cap_source = np.rint(fields["WallStage8eCapSource"]).astype(int)
    limiter_class = np.rint(fields["WallStage8eLimiterClass"]).astype(int)
    ratio_cap_selected = active & (cap_source == 2)
    abs_cap_selected = active & (cap_source == 1)
    cap_demand_ratio = fields["WallStage8eCapDemandRatio"]
    ...
    add_group("cap_source", [("abs_cap_selected", cap_source == 1), ("ratio_cap_selected", cap_source == 2), ("none_or_inactive", cap_source == 0)])
    add_group(
        "limiter_class",
        [
...
def decision(rows: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    return {
        ...
        "block_reason": "Stage8f is diagnostic-only; sphere Stage8OperatorMode=2 remains forbidden",
...
def main() -> None:
    ...
    (args.out_dir / "stage8f_normal_limiter_attribution_summary.json").write_text(
        json.dumps(payload, indent=2), encoding="utf-8"
    )
    write_csv(args.out_dir / "stage8f_normal_limiter_attribution_summary.csv", [flatten_summary(r) for r in summaries])
    write_csv(args.out_dir / "stage8f_normal_limiter_attribution_bins.csv", bin_rows_all)
