#!/usr/bin/env python3
"""Build curated Stage8g shadow gate summaries from attribution artifacts."""
        ...
        out.append(
            {
                ...
                "outer90_limiter_total": sum(int(r.get("outer90_normal_limiter_count") or 0) for r in subset),
...
def main() -> None:
    ...
    parser.add_argument("--out-dir", type=Path, default=Path("artifacts/stage8g_cap_contract_revision_summary_20260613"))
    args = parser.parse_args()
    repo = args.repo_root.resolve()
    out_dir = args.out_dir if args.out_dir.is_absolute() else repo / args.out_dir
    out_dir.mkdir(parents=True, exist_ok=True)
    flat_root = repo / "artifacts" / "flat_wall_cap_stage8g_low_angle_20260613"
    sphere_root = repo / "artifacts" / "pre2025_sphere_stage8g_shadow_20260613"
    flat = final_rows(flat_root, "*/analysis_stage8g_attribution/stage8g_shadow_attribution_summary.json")
    sphere = final_rows(sphere_root, "*/analysis_stage8g_attribution/stage8g_shadow_attribution_summary.json")
    write_csv(out_dir / "stage8g_flat_shadow_summary.csv", flat)
    write_csv(out_dir / "stage8g_sphere_shadow_summary.csv", sphere)
    summary = {
        "status": STATUS,
        "claim_limit": "runtime_sanity / exploratory_not_validation only",
        "decision": "Stage8g is shadow-only; sphere Stage8OperatorMode=2 remains forbidden",
    ...
    (out_dir / "stage8g_gate_summary.json").write_text(json.dumps(clean(summary), indent=2), encoding="utf-8")
