#!/usr/bin/env python3
"""Generate Stage8h flat-wall contact-relation/profile-path shadow cases."""
...
CASE_DIR = Path("cases") / "diagnostics" / "flat_wall_cap_stage8h_contact_relation_20260613"
REMOTE_ROOT = "/media/yuan/新加卷1/RUNS/runs/stage8/stage8h_contact_relation_and_profile_path_audit_20260613/flat"
BINARY = (
...
STAGE8H_VTK = (
    ...
    "WallStage8gMode,WallStage8gScaleRawNormal,WallStage8gScaleTarget,"
    "WallStage8gScaleTangent,WallStage8gScaleFloor,WallStage8gEffectiveScale,"
    "WallStage8gTanRaw,WallStage8gTanEff,WallStage8gRegularizationRatio,"
    "WallStage8gCapSource,WallStage8gCapDemandRatio,"
    "WallStage8gProfileTargetMismatch,WallStage8gProfileConflictSign,"
    "WallStage8gWriteAllowedFlag,"
    "WallStage8hMode,WallStage8hActualCos,WallStage8hTargetCos,"
    "WallStage8hResidualCos,WallStage8hDnTanRaw,WallStage8hDnCosRaw,"
    "WallStage8hDnRelaxed,WallStage8hBetaRelaxation,"
    "WallStage8hBetaSource,WallStage8hProfileNormal,"
    "WallStage8hProfileTargetMismatch,WallStage8hProfileConsistencyWeight,"
    "WallStage8hCandidateDemandRatio,WallStage8hCandidateNormalDelta,"
    "WallStage8hLimiterEquivalent,WallStage8hCosToTanRatio,"
    "WallStage8hEffectiveCap,WallStage8hWriteAllowedFlag"
)
...
def cases_for_set() -> list[FlatCase]:
    return [
        FlatCase(f"cap_theta030_wall{angle:03d}_stage8h_mode{mode}", 30.0, float(angle), mode)
...
<!--
  Stage8h flat-wall contact-relation/profile-path shadow diagnostic.
  Status: {STATUS} / exploratory_not_validation.
  Stage8OperatorMode=1 shadow-only; no gradPhiVal write.
-->
<CLBConfig version="2.0" output="{remote_case}/output/" permissive="true">
  ...
  <Model>
    ...
    <Param name="Stage8hMode" value="{case.stage8h_mode}"/>
...
def main() -> None:
    ...
    for case in cases_for_set():
        ...
        params = {
            ...
            "stage8g_mode": 3,
            "stage8h_mode": case.stage8h_mode,
    ...
    manifest = {
        ...
        "purpose": "Stage8h flat-wall contact-relation/profile-path shadow gate",
        "binary": args.binary,
