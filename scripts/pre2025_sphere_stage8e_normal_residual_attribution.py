#!/usr/bin/env python3
"""Postprocess Stage8e normal-residual-only shadow attribution VTI frames."""
...
def decision(rows: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    return {
        ...
        "write_mode_allowed": not bool(blocked),
        "write_mode_blocked": bool(blocked),
        "block_reason": "Stage8e shadow gate failed; sphere Stage8OperatorMode=2 remains forbidden" if blocked else "Stage8e shadow gate passed for short write planning only",
...
def main() -> None:
    ...
    (args.out_dir / "stage8e_normal_residual_attribution_summary.json").write_text(
        json.dumps(payload, indent=2), encoding="utf-8"
    )
    write_csv(args.out_dir / "stage8e_normal_residual_attribution_summary.csv", [flatten_summary(r) for r in summaries])
    write_csv(args.out_dir / "stage8e_normal_residual_attribution_bins.csv", bin_rows_all)
