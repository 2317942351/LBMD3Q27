...
# Geometry-isolation control for the PRE 2025 reduced sphere theta030 profile lane.
# Status: exploratory_not_validation. Raw VTI/PVTI/PRI remain on HM570.

RUN_ROOT="${1:-/mnt/8A0E24070E23EAC1/runs/tclb_pre2025_sphere_theta030_profile_radAngle011_M0p1_W6_liftZ32_400k_20260610}"
...
mkdir -p "$CASE_DIR"
batch_log="$RUN_ROOT/batch_profile_liftZ32_400k.log"
current_case_dir=""
...

{
  ...
  echo "status=exploratory_not_validation"
  echo "purpose=geometry_isolation_lift_solid_sphere_from_zmin_wall"
...
for helper in "$POSTPROCESS" "$WALL_POSTPROCESS" "$GALLERY_SCRIPT" "$SURFACE_FILM_AUDIT"; do
  if [[ ! -f "$helper" ]]; then
...
rm -f "$CASE_DIR/analysis_surface_film_zmin.returncode"
rm -rf "$CASE_DIR/analysis_pre2025_sphere" "$CASE_DIR/analysis_wall_diag"
rm -rf "$CASE_DIR/analysis_morphology" "$CASE_DIR/analysis_surface_film_zmin"

echo "CASE_START $(date -Is) theta=030 liftZ32" | tee -a "$batch_log"
set +e
"$BINARY" "$CASE_DIR/case.xml" > "$CASE_DIR/run.log" 2> "$CASE_DIR/run.stderr"
rc=$?
set -e
printf '%s\n' "$rc" > "$CASE_DIR/run.returncode"
echo "CASE_END $(date -Is) theta=030 liftZ32 rc=$rc" | tee -a "$batch_log"
...
if grep -Eiq '(^|[^A-Za-z])nan([^A-Za-z]|$)|(^|[^A-Za-z])inf([^A-Za-z]|$)|Stopping due to NaN|discovered NaN|NaN value|Checking .*discovered|error[[:space:]]*!' "$CASE_DIR/run.log" "$CASE_DIR/run.stderr"; then
  ...
  echo "CASE_NUMERICAL_FAILURE $(date -Is) theta=030 liftZ32" | tee -a "$batch_log"
  exit 6
fi
if [[ "$rc" -ne 0 ]]; then
  exit "$rc"
...
python3 "$GALLERY_SCRIPT" \
  ...
  --solid-center-z "$SOLID_CENTER_Z" \
  --solid-radius "$SOLID_RADIUS" \
  --title "profile theta030 radAngle11 M0.1 W6, lifted solid z=32, 0-400k every 50k" \
...
mkdir -p "$CASE_DIR/analysis_surface_film_zmin"
set +e
python3 "$SURFACE_FILM_AUDIT" \
  --case-root "$CASE_DIR" \
  --out-dir "$CASE_DIR/analysis_surface_film_zmin" \
  --analysis-subdir analysis_pre2025_sphere \
  --solid-center-x "$SOLID_CENTER_X" \
  --solid-center-y "$SOLID_CENTER_Y" \
  --solid-center-z "$SOLID_CENTER_Z" \
  ...
  --bottom-plane-zmax 8 \
  --plot \
  > "$CASE_DIR/analysis_surface_film_zmin_stdout.log" \
  2> "$CASE_DIR/analysis_surface_film_zmin_stderr.log"
film_rc=$?
set -e
printf '%s\n' "$film_rc" > "$CASE_DIR/analysis_surface_film_zmin.returncode"
echo "SURFACE_FILM_END $(date -Is) rc=$film_rc" | tee -a "$batch_log"
if [[ "$film_rc" -ne 0 ]]; then
  exit "$film_rc"
fi

tar -czf "$RUN_ROOT/curated_profile_liftZ32_400k_theta030.tar.gz" \
  --exclude='*.vti' --exclude='*.pvti' --exclude='*.pri' \
  -C "$RUN_ROOT" theta030 batch_profile_liftZ32_400k.log
