...
# Run Stage8e flat-wall low-angle diagnostics.
# Status: runtime_sanity / exploratory_not_validation.
# Raw VTI/PVTI/PRI stay on HM570; no archive is produced here.

RUN_ROOT="${1:-/mnt/8A0E24070E23EAC1/runs/tclb_flat_wall_cap_stage8e_low_angle_20260613}"
LOCAL_CASE_STAGING="${LOCAL_CASE_STAGING:-/tmp/flat_wall_cap_stage8e_low_angle_cases}"
...
mkdir -p "$RUN_ROOT"
batch_log="$RUN_ROOT/batch_flat_wall_cap_stage8e.log"
current_case_dir=""
...

{
  ...
  echo "status=runtime_sanity"
  echo "purpose=flat_wall_spherical_cap_gate_stage8e_normal_residual"
